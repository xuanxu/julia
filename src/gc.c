...
static int64_t per_thread_counter_interval(int tid, size_t interval)
{
    if (jl_n_threads == 1)
        return interval;
    // stagger global counter updates over threads, starting at some minimum (1MB),
    // in varying amounts that add to `interval`.
    size_t min = 1048576;
    return tid * 2 * (interval/jl_n_threads - min) / (jl_n_threads - 1) + min;
}
...
static inline int maybe_collect(jl_ptls_t ptls)
{
    ...
    if (ptls->gc_num.allocd >= 0) {
        ...
        should_collect = (gc_num.allocd > 0);
    }
...
static void combine_thread_gc_counts(jl_gc_num_t *dest)
{
    for (int i = 0; i < jl_n_threads; i++) {
        ...
        if (ptls) {
            dest->allocd += (ptls->gc_num.allocd + per_thread_counter_interval(ptls->tid, gc_num.interval));
            dest->freed += ptls->gc_num.freed;
            dest->malloc += ptls->gc_num.malloc;
            dest->realloc += ptls->gc_num.realloc;
            dest->poolalloc += ptls->gc_num.poolalloc;
            dest->bigalloc += ptls->gc_num.bigalloc;
            dest->freecall += ptls->gc_num.freecall;
        }
    }
}
...
static void reset_thread_gc_counts(void)
{
    for (int i = 0; i < jl_n_threads; i++) {
        ...
        if (ptls) {
            ...
            ptls->gc_num.allocd = -per_thread_counter_interval(ptls->tid, gc_num.interval);
        }
    }
}
...
void jl_init_thread_heap(jl_ptls_t ptls)
{
    ...
    memset(&ptls->gc_num, 0, sizeof(jl_thread_gc_num_t));
    ptls->gc_num.allocd = -per_thread_counter_interval(ptls->tid, default_collect_interval);
}
